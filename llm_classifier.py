...
BATCH_SIZE    = 3
TOP_N_ENTRIES = 30   # small batches - free tier limit is 8K tokens/min
...
def _top_entries(raw_names, library_rows, n=TOP_N_ENTRIES):
    query = " ".join(str(x).lower() for x in raw_names)
    scored = []
    for r in library_rows.itertuples():
        if str(r.publisher).startswith("("):
            continue
        cand = f"{r.publisher} {r.product_family}".lower()
        scored.append((SequenceMatcher(None, query, cand).ratio(), r))
    scored.sort(key=lambda x: -x[0])
    return [r for _, r in scored[:n]]

def _build_prompt(raw_names: list, library_rows) -> str:
    library_rows = __import__("pandas").DataFrame([r._asdict() for r in _top_entries(raw_names, library_rows)])
