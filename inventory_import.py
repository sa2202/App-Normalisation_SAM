    ...
    for _, row in df.iterrows():
        ...
        if matched_pub_raw.startswith("("):
            ...
            if publishers_match(matched_pub_raw, source_vendor):
                ...
                # A library match that contradicts the source Vendor column is
                # USUALLY a bad fuzzy match worth discarding. But not always -
                # the vendor column itself can be wrong, or schema detection can
                # pick the wrong column entirely. Two guards prevent that from
                # corrupting good matches:
                #
                #   1. EXACT matches are never overridden. An exact alias hit is
                #      stronger evidence than a vendor string, so a disagreement
                #      means the VENDOR DATA is suspect, not the match. Flag it
                #      and move on.
                #   2. The replacement must be atomic and complete. Previously
                #      publisher could be overwritten while product/edition kept
                #      library values, producing impossible rows like
                #      "Cohesity | Creative Cloud | Media Encoder" - a
                #      Frankenstein of two different products. Now every product
                #      field moves together, or none does.
                vendor_check = "mismatch"

                # Guard 1a: if the library-matched product name actually appears
                # in the raw input text, the match is corroborated by the data
                # itself - the vendor column is the odd one out. E.g. raw
                # "Media Encoder CC (2023)" matched to Adobe "Media Encoder":
                # the product name is right there in the string, so a Vendor
                # column saying "Cohesity" is wrong, not the match.
                matched_fam = str(result.get("product_family") or "").lower()
                raw_l = raw_name.lower()
                corroborated = bool(matched_fam) and len(matched_fam) >= 4 and matched_fam in raw_l

                if result["confidence_tier"] == "EXACT" or corroborated:
                    # Trust the match, distrust the vendor column.
                    vendor_check = "mismatch (match kept - product name corroborated by raw text)"
                else:
                    from product_parser import parse_product
                    parsed = parse_product(raw_name, source_vendor)
                    # Guard 2: only swap if the parse produced a COMPLETE,
                    # trustworthy alternative - both a publisher and a product.
                    # A half-parse is worse than the original match.
                    if parsed["product"] and parsed["publisher"]:
                    ...
                    elif result["confidence_tier"] == "HIGH":
                        # Can't produce a better answer - keep the match but
                        # drop confidence so a human looks at it.
                        result["confidence_tier"] = "REVIEW"
                        vendor_check = "mismatch (downgraded to REVIEW)"
