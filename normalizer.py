...
# ── rolling-release families ──────────────────────────────────────────────
# Several vendors ship products on a near-continuous build-number cadence
# rather than discrete versions - confirmed from real ARL data: Citrix
# Enterprise Browser (97-140+), Workspace App, Secure Access, Microsoft Teams
# (23000-26000+), Edge (100-153+), Copilot in Edge. Enumerating one canonical
# entry per build is a losing battle - a new build ships roughly monthly.
# Instead, match "<family phrase> <number(.number)*>" as a whole to ONE
# family entry, regardless of the specific build number. The number itself
# carries no product-identity information for these families - only the
# family name does.
ROLLING_FAMILIES = [
    # (regex matching the cleaned/expanded text, canonical_id)
    # Version numbers use \d+((\.|\s)\d+)* because clean_text() turns
    # punctuation (including '.') into spaces before this stage runs -
    # "25.11" becomes "25 11", so the pattern must accept either separator.
    (r"^citrix enterprise browser \d+((\.|\s)\d+)*$", "CITRIX-ENTERPRISE-BROWSER"),
    (r"^enterprise browser \d+((\.|\s)\d+)*$",         "CITRIX-ENTERPRISE-BROWSER"),
    (r"^citrix workspace app \d+((\.|\s)\d+)*$",       "CITRIX-WORKSPACE-APP"),
    (r"^workspace app \d+((\.|\s)\d+)*$",               "CITRIX-WORKSPACE-APP"),
    (r"^citrix secure access \d+((\.|\s)\d+)*$",       "CITRIX-SECURE-ACCESS"),
    (r"^secure access \d+((\.|\s)\d+)*$",               "CITRIX-SECURE-ACCESS"),
    (r"^microsoft teams \d+((\.|\s)\d+)*$",             "MS-TEAMS-ROLLING"),
    (r"^teams \d{4,6}$",                                 "MS-TEAMS-ROLLING"),
    (r"^microsoft edge \d+((\.|\s)\d+)*$",              "MS-EDGE-ROLLING"),
    (r"^edge \d+((\.|\s)\d+)*$",                         "MS-EDGE-ROLLING"),
    (r"^copilot in edge \d+((\.|\s)\d+)*$",             "MS-COPILOT-EDGE-ROLLING"),
]
_ROLLING_RE = [(re.compile(p), cid) for p, cid in ROLLING_FAMILIES]
...
def strip_version_suffix(text: str) -> str:
    """
    Registry DisplayName entries often append a dotted patch/build number in
    parentheses to an otherwise-exact product name, e.g. 'Adobe Reader XI
    (11.0.10)'. That's the SAME product as 'Adobe Reader XI' - the suffix is
    a patch identifier, not a different product. Strip it before matching so
    these hit EXACT instead of falling to fuzzy/REVIEW.
    Only strips DOTTED version-looking parens (11.0.10) - NOT short
    qualifiers like '(64-bit)' or '(x64)', which carry real signal.
    """
    if not isinstance(text, str):
        return text
    return re.sub(r"\s*\(\d+(\.\d+){1,4}\)\s*$", "", text).strip()


def clean_text(raw: str) -> str:
    if not isinstance(raw, str): return ""
    t = strip_version_suffix(raw)
    t = t.lower()
    t = re.sub(r"[\-\_/\\]", " ", t)
    t = re.sub(r"[^\w\s]", " ", t)
...
class ProductNormalizer:
    ...
    def _build_index(self):
        ...
        self.fuzzy_corpus   = []
        self.family_corpus  = []
        self._id_lookup     = set(self.lib["canonical_id"])
    ...
    def classify(self, raw_name: str, vendor_hint: str = "") -> dict:
        ...
        # Strip patch-number suffixes like '(11.0.10)' BEFORE metric extraction,
        # which otherwise strips the parens first and leaves the digits behind
        # as unremovable noise (e.g. 'Adobe Reader XI 11.0.10').
        raw_name = strip_version_suffix(raw_name)
        ...
        # ── Stage 0: rolling-release family (before exact match) ──────
        for rx, family_cid in _ROLLING_RE:
            if rx.match(expanded) and family_cid in self._id_lookup:
                return self._result(original, family_cid, "HIGH", 95,
