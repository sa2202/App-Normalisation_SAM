...
if st.session_state.results is not None and "File evidence" not in mode:
    ...
    def build_excel(results: pd.DataFrame) -> bytes:
        ...
        with pd.ExcelWriter(buf, engine="openpyxl") as w:
            ...
            # ── Sheet 1: Normalised ───────────────────────────────────
            s1_cols = [c for c in [
                "raw_input", "publisher", "product_family", "edition", "version",
                "classification", "confidence_tier", "match_score",
                "metric_type", "canonical_id", "source_type", "non_licensable",
            ] if c in results.columns]
            results[s1_cols].to_excel(w, index=False, sheet_name="Normalised")
            ws1 = w.sheets["Normalised"]

            tier_colours = {
                "EXACT": "C6EFCE", "HIGH": "FFEB9C", "REVIEW": "FFCC99",
                "PARSED": "BDD7EE", "UNRESOLVED": "FFC7CE",
            }
            for cell in ws1[1]:
                cell.fill = PatternFill("solid", fgColor="1F4E79")
                cell.font = Font(bold=True, color="FFFFFF")
                cell.alignment = Alignment(horizontal="center", wrap_text=True)
            tier_idx = next((i for i, c in enumerate(ws1[1], 1)
                             if "Confidence" in str(c.value)), None)
            if tier_idx:
                for row in ws1.iter_rows(min_row=2):
                    colour = tier_colours.get(str(row[tier_idx - 1].value or ""))
                    if colour:
            ...
            # ── Sheet 2: Summary (commercial products only) ───────────
            commercial = results[
                ~results["publisher"].fillna("").str.startswith("(")
            ].copy()
            if "classification" in commercial.columns:
                commercial = commercial[
                    commercial["classification"].fillna("").isin(["Commercial", ""])
                ]

            grp_cols = [c for c in [
                "publisher", "product_family", "edition", "version",
                "classification", "metric_type",
            ] if c in commercial.columns]

            if len(commercial):
                summary = (
                    commercial.groupby(grp_cols, dropna=False)
                    .agg(
                        device_count=("raw_input", "count"),
                        confidence=("confidence_tier",
                                    lambda s: ", ".join(
                                        f"{k}:{v}" for k, v in s.value_counts().items())),
                    )
                    .reset_index()
                    .sort_values(["publisher", "product_family"])
                )
            else:
                summary = pd.DataFrame({"Note": ["No commercial products found"]})
            ...
            # ── Sheet 3: Product Count Chart ──────────────────────────
            if len(commercial):
                chart_df = (
                    commercial
                    .groupby(["publisher", "product_family"], dropna=False)
                    .agg(count=("raw_input", "count"))
                    .reset_index()
                    .sort_values("count", ascending=False)
                    .head(20)
                )
                chart_df["Product"] = (
                    chart_df["publisher"].fillna("").str.split("(").str[0].str.strip()
                    + " – " + chart_df["product_family"].fillna("")
                )
                chart_df = chart_df[["Product", "count"]].rename(
                    columns={"count": "Device Count"})
            else:
                chart_df = pd.DataFrame({"Product": ["No data"], "Device Count": [0]})
            ...
            chart.type    = "bar"
            chart.barDir  = "bar"
            chart.title   = "Product Deployment Count (Commercial only, top 20)"
            chart.x_axis.title = "Number of Devices"
            chart.style   = 10
            chart.width   = 24
            chart.height  = 16
