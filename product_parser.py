...
PUBLISHER_ALIASES = {
    ...
    "sap": ["sap", "sap se", "sap ag", "sap america", "sybase"],
    # Acquisitions - the acquired brand and the acquirer must match each other,
    # otherwise a vendor column saying "Cohesity" fails against a library
    # publisher of "Veritas (Cohesity)" and triggers a false mismatch.
    "veritas": ["veritas", "cohesity", "veritas technologies", "veritas cohesity"],
    "citrix": ["citrix", "citrix systems", "cloud software group"],
    "symantec": ["symantec", "broadcom", "symantec corporation"],
    "ca technologies": ["ca technologies", "ca inc", "ca", "broadcom"],
    "splunk": ["splunk", "cisco", "splunk inc"],
    "tableau": ["tableau", "salesforce", "tableau software"],
    "slack": ["slack", "salesforce", "slack technologies"],
    "github": ["github", "microsoft"],
    "hashicorp": ["hashicorp", "ibm"],
    "mcafee": ["mcafee", "trellix"],
    "nuance": ["nuance", "microsoft"],
    "kofax": ["kofax", "tungsten automation"],
}
...
def parse_product(raw_name: str, vendor_hint: str = "") -> dict:
    ...
    # Handles acquisition-style publishers too: a library publisher like
    # "Veritas (Cohesity)" must not survive inside the product name as
    # "Veritas (Cohesity) Backup Exec" - strip the bare name, the
    # parenthetical form, and any acquirer alias.
    if publisher and text:
        strip_forms = [publisher]
        bare = re.sub(r"\(.*?\)", "", publisher).strip()
        if bare and bare != publisher:
            strip_forms.append(bare)
        inner = re.findall(r"\((.*?)\)", publisher)
        strip_forms.extend(i.strip() for i in inner if i.strip())
        # Longest first so "Veritas (Cohesity)" is tried before "Veritas"
        for form in sorted(set(strip_forms), key=len, reverse=True):
            if form and text.lower().startswith(form.lower() + " "):
